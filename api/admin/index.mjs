...
(function(){
  function countryName(code){
    ...
    try{
      if (typeof Intl !== "undefined" && Intl.DisplayNames) {
        const dn = new Intl.DisplayNames(['ro'], { type: 'region' });
        return dn.of(code) || code;
      }
      return code;
    }catch { return code; }
  }
  function tnum(n, cur){ return (n ?? 0) + " " + String(cur||"").toUpperCase(); }
  function fmtDate(ts){
    if(!ts) return "-";
    try{
      const d = new Date(ts);
      const dS = d.toLocaleDateString('ro-RO', { day:'2-digit', month:'2-digit', year:'numeric' });
      const tS = d.toLocaleTimeString('ro-RO', { hour:'2-digit', minute:'2-digit' });
      return dS + ", " + tS;
    }catch{ return "-"; }
  }
  function formatsFrom(items){
    const f = Array.from(new Set((items||[]).map(i => String(i.format||"").toUpperCase()).filter(Boolean)));
    return f.join(", ") || "-";
  }
  function typeFromItems(items){
    const hasE = (items||[]).some(i => String(i.format||"").toUpperCase() !== "PAPERBACK");
    const hasP = (items||[]).some(i => String(i.format||"").toUpperCase() === "PAPERBACK");
  ...
  async function load(){
    ...
    try{
      ...
      if(!t){
        root.innerHTML = '<p style="color:#b42318">Introdu tokenul admin mai sus.</p>';
        return;
      }
      ...
      if(!res.ok){
        root.innerHTML = '<p style="color:#b42318">Eroare la încărcare: ' + res.status + '</p>';
        return;
      }
      ...
      if (cSel && cSel.getAttribute('data-init') !== '1') {
      ...
      orders = orders.filter(o => {
        ...
        if (c && String(o.currency||'').toUpperCase() !== c) return false;
        if (k && String(o.country||'').toUpperCase() !== k) return false;
        if (f) {
      ...
      if(!orders.length){
        root.innerHTML = '<div class="muted" style="padding:16px;background:#fff;border:1px solid var(--line);border-radius:12px;">Fără comenzi pentru filtrele curente.</div>';
        return;
      }
      ...
      const rows = orders.map(o => {
        ...
        const name = o.name || '';
        const id = o.id || '';
        const total = tnum(o.amount || 0, o.currency || '');
        ...
        const fmts = formatsFrom(o.items);
        ...
        return \`
          <tr class="row">
            ...
            <td>\${name ? name : '<span class="muted">-</span>'}<br/><a href="mailto:\${email}">\${email}</a></td>
            <td class="country">\${country || '-' }<br/><span class="tiny">\${country ? countryName(country) : ''}</span></td>
      ...
      root.innerHTML = \`
        <table>
          <thead>
            <tr>
              <th>ID / Status</th><th>Data</th><th>Client</th><th>Țară</th>
              <th style="min-width:320px">Produse</th><th>Format(e)</th><th class="right">Total</th><th>Tip</th>
            </tr>
          </thead>
          <tbody>\${rows}</tbody>
        </table>\`;
      ...
      root.innerHTML = '<p style="color:#b42318">Eroare script: ' + (e?.message || e) + '</p>';
    }
  }
