...
<style>
  ...
  table { width:100%; border-collapse:collapse; background:#fff; border-radius:10px; overflow:hidden; box-shadow:0 4px 14px rgba(0,0,0,0.05); }
  th, td { padding:10px 12px; border-bottom:1px solid #eee; vertical-align:top; font-size:14px; }
  ...
  .debug { margin:8px 0 14px; font:12px/1.5 system-ui; color:#666; }
</style>
</head>
<body>
  ...
  <div class="debug">Citesc din: <code id="dbg-url"></code></div>
...
async function load() {
  ...
  document.getElementById('dbg-url').textContent = url;
  ...
  try {
    ...
    if (!orders.length) {
      document.getElementById('root').innerHTML = '<div class="no-orders">Fără comenzi înregistrate.</div>';
      return;
    }
    const rows = orders.map(o => {
      const when = new Date(o.createdAt || 0).toLocaleString('ro-RO');
      const items = (o.items||[]).map(it => 
        \`\${it.description || (it.title || '')} × \${it.quantity} — \${(it.amount_total||0)} \${(it.currency||'').toUpperCase()}\`
      ).join('<br/>');
      return \`
        <tr>
    ...
    document.getElementById('root').innerHTML = \`
      <table>
        <thead>
          <tr>
            <th>ID</th><th>Data</th><th>Client</th><th>Produse</th><th>Total</th><th>Tip</th>
          </tr>
        </thead>
        <tbody>\${rows}</tbody>
      </table>\`;
  } catch (e) {
    document.getElementById('root').innerHTML = '<p style="color:#b42318">Network error: ' + (e && e.message) + '</p>';
  }
}
